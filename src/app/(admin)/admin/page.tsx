  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const revenue = ordersData.filter((o: any) => o.status === 'Completed' || o.payment_status === 'Paid').reduce((acc: number, o: any) => acc + Number(o.total_price), 0);
    ...
    <div className="w-full max-w-6xl mx-auto py-6 space-y-6">
      ...
      <div className="bg-zinc-900/80 border border-zinc-800 backdrop-blur-md shadow-lg rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white tracking-tight">Welcome, Admin!</h2>
            <p className="text-zinc-400 mt-1">Here is the summary of KopiKuy&apos;s performance today.</p>
          </div>
        </div>
      ...
      <div className="bg-zinc-900/80 border border-zinc-800 backdrop-blur-md shadow-lg rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Clock size={18} className="text-amber-500" /> Recent Transactions
          </h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-zinc-800/50">
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-zinc-500 animate-pulse">Loading recent orders...</td>
                </tr>
                ...
                recentOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-zinc-800/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 font-semibold text-amber-500">
                      Rp {Number(order.total_price).toLocaleString('en-US')}
                    </td>
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-zinc-500">No incoming orders yet.</td>
                </tr>
              )}
            </tbody>
          </table>
